...
namespace App04.MultiLang.Helper;
...
public class IniFileHelper
{
    ...
    /// <see>
    ///     <cref>https://msdn.microsoft.com/zh-cn/library/ms724353.aspx</cref>
    /// </see>
    /// <returns>返回取得字符串缓冲区的长度</returns>
    [DllImport("kernel32")]
    private static extern int GetPrivateProfileString(byte[] sectionName, byte[] key, byte[] defaultValue,
    ...
    /// <param name="filePath">ini文件的完整路径和文件名</param>
    /// <see>
    ///     <cref>https://msdn.microsoft.com/zh-cn/library/ms725501.aspx</cref>
    /// </see>
    ...
    public static string GetValue(string sectionName, string key, string filePath)
    {
        ...
        var length = GetPrivateProfileString(Encoding.UTF8.GetBytes(sectionName), Encoding.UTF8.GetBytes(key),
            Encoding.UTF8.GetBytes("未填写"), buffer, 255, filePath);

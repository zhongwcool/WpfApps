...
public class VisualStateGroupListener : FrameworkElement
{
    ...
    {
        VisibilityProperty.OverrideMetadata(typeof(VisualStateGroupListener),
    ...
    {
        if (oldGroup != null) oldGroup.CurrentStateChanged -= OnCurrentStateChanged;
    ...
    {
        if (e.OldValue is VisualStateGroupListener oldListener) oldListener.ClearValue(GroupProperty);

        if (e.NewValue is VisualStateGroupListener newListener) newListener.Group = (VisualStateGroup)d;
    }

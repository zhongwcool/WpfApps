...
public static class FocusVisualHelper
{
    ...
    private static void OnShowFocusVisualChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        if (d is Control control && GetTemplateFocusTarget(control) is { } target)
        {
            if ((bool)e.NewValue)
            {
                ...
                if (shouldShowFocusVisual) ShowFocusVisual(control, target);
            }
            else
            {
                HideFocusVisual();
        ...
        static void HideFocusVisual()
        {
            ...
            if (_focusVisualAdornerCache != null)
            {
                ...
                if (adornerlayer != null) adornerlayer.Remove(_focusVisualAdornerCache);
    ...
    {
        if (!source.HasDefaultValue(dp)) target.SetValue(dp, source.GetValue(dp));
    }

    private sealed class FocusVisualAdorner : Adorner
    {
        public FocusVisualAdorner(Control focusedElement, UIElement adornedElement, Style focusVisualStyle) : base(
            adornedElement)
        {
    ...
    private static FocusVisualAdorner _focusVisualAdornerCache;
}
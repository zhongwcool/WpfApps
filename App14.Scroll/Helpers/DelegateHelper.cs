...
namespace App14.Scroll.Helpers;
...
internal static class DelegateHelper
{
        ...
        where T : Delegate
    {
        if (bindingAttr != DefaultLookup)
        {
            ...
            if (methodInfo != null) return CreateDelegate<T>(methodInfo);
            return null;
        }
        ...
        where T : Delegate
    {
        if (bindingAttr != DefaultLookup)
        {
            ...
            if (methodInfo != null) return CreateDelegate<T>(target, methodInfo);
            return null;
        }
    ...
    public static Func<TType, TProperty> CreatePropertyGetter<TType, TProperty>(string name,
        BindingFlags bindingAttr = DefaultLookup, bool nonPublic = false)
    {
        var property = typeof(TType).GetProperty(name, bindingAttr);
        if (property != null)
        {
            var getMethod = property.GetGetMethod(nonPublic);
            if (getMethod != null) return CreateDelegate<Func<TType, TProperty>>(getMethod);
        }

        return null;
    }

    public static Action<TType, TProperty> CreatePropertySetter<TType, TProperty>(string name,
        BindingFlags bindingAttr = DefaultLookup, bool nonPublic = false)
    {
        var property = typeof(TType).GetProperty(name, bindingAttr);
        if (property != null)
        {
            ...
            if (setMethod != null) return CreateDelegate<Action<TType, TProperty>>(setMethod);
        }

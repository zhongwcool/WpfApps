...
using App14.Scroll.Helpers;

namespace App14.Scroll.Manager;
...
public class SimpleVisualStateManager : VisualStateManager
{
    ...
    internal static bool TryGetState(IList<VisualStateGroup> groups, string stateName, out VisualStateGroup group,
    ...
    private bool GoToStateInternal(FrameworkElement control, FrameworkElement stateGroupsRoot, VisualStateGroup group,
        VisualState state, bool useTransitions)
    {
        if (stateGroupsRoot == null) throw new ArgumentNullException("stateGroupsRoot");
    ...
    /// </returns>
    internal static VisualTransition GetTransition(FrameworkElement element, VisualStateGroup group, VisualState from,
        VisualState to)
    {
        ...
        if (transitions != null)
            foreach (var transition in transitions)
            {
                ...
                if (from == transitionFromState)
                    score += 1;
                else if (transitionFromState != null) continue;

                if (to == transitionToState)
                    score += 2;
                else if (transitionToState != null) continue;

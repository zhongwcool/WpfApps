...
internal class AudioDataAnalyzer
{
    ...
    private readonly int _fftWindowSize;
    ...
    public AudioDataAnalyzer(AudioDeviceHandler handler)
    {
        ...
        for (var i = 0; i < FftIndices.Length; i++) FftIndices[i] = i * FftFrequencySpacing;

        SpectrogramBuffer = new double[SpectrogramFrameCount, FftDataPoints];
        for (var x = 0; x < SpectrogramFrameCount; x++)
        for (var y = 0; y < FftDataPoints; y++)
            SpectrogramBuffer[x, y] = double.MinValue;
        DbValues = new double[FftDataPoints];

        PrimaryIndices = new int[handler.BufferSize];
        for (var i = 0; i < PrimaryIndices.Length; i++) PrimaryIndices[i] = i;
    ...
    private void ProcessData(double[] input)
    {
        ...
        for (var i = 0; i < FftDataPoints; i++) SpectrogramBuffer[SpectrogramFrameCount - 1, i] = DbValues[i];
